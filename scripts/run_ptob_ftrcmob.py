...
def run_ptob_ftrcmob_tabular(base_cfg):
    ...
    for train_idx_outer, test_idx_outer in skf_outer.split(X_all, y_all):
        ...
        # NOTE: Pre-training OB
        train_loader = data_loader(X_train_run, y_train, batch_size=params["train"]["batch_size"], num_workers=params["num_workers"], shuffle=True)
...
def run_ptob_ftrcmob_image(base_cfg):
    if base_cfg['base_method_name'] != 'OBDECOC':
        raise ValueError("Only OBDECOC is supported as base method for PTRCM + FTRCM-OBD.")

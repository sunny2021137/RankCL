    ...
    for train_idx_outer, test_idx_outer in skf_outer.split(X_all, y_all):
        ...
        def objective(trial):
            ...
            for train_idx, val_idx in skf.split(X_train_full, y_train_full):
                ...
                pre_trainer = BaselineTrainer(params, train_loader, val_loader, test_loader, n_classes, x_dim, is_pretrain=True)
                # train
                pre_trainer.train()
                ...
                # CHANGE: 2
                pretrained_model_dict = {k: v for k, v in pretrained_model_dict.items() if k in finetune_model_dict}
                # print("left:")
                # for k, v in pretrained_model_dict.items():
                #     print(f"{k}")
...
def run_ptob_ftrcmob_optuna_image(config):
    if config['base_method_name'] != 'OBDECOC':
        raise ValueError("Only OBDECOC is supported as base method for PTRCM + FTRCM-OBD.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Optuna Hyperparameter Optimization (PT-OB + FT-RCM-OB)")
    parser.add_argument("--config", type=str, default="configs/optuna/default_optuna_ptob_ftrcmob.yaml")
    ...
    if opt_cfg["dataset"]["dataset_type"] == "tabular":
        run_ptob_ftrcmob_optuna_tabular(opt_cfg)
    elif opt_cfg["dataset"]["dataset_type"] == "image":
        run_ptob_ftrcmob_optuna_image(opt_cfg)
    else:

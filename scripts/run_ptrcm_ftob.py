...
import argparse
from train.trainer import BaselineTrainer, RankCLPretrainedTrainer
...
def run_ptrcm_ftob_tabular(base_cfg):
...
def run_ptrcm_ftob_image(base_cfg):
    if base_cfg['base_method_name'] != 'OBDECOC':
        raise ValueError("Only OBDECOC is supported as base method for PTRCM + FTRCM-OBD.")
...
def main():
    parser = argparse.ArgumentParser(description="PT-RCM + FT-OB")
    parser.add_argument("--config", type=str, default="configs/default_ptrcm_ftob.yaml")

...
def run_ptrcm_ftrcmob_optuna_tabular(config):
...
def run_ptrcm_ftrcmob_optuna_image(config):
    if config['base_method_name'] != 'OBDECOC':
        raise ValueError("Only OBDECOC is supported as base method for PTRCM + FTRCM-OBD.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Optuna Hyperparameter Optimization (PT-RCM + FT-RCM-OB)")
    parser.add_argument("--config", type=str, default="configs/optuna/default_optuna_ptrcm_ftrcmob.yaml")
    ...
    if opt_cfg["dataset"]["dataset_type"] == "tabular":
        run_ptrcm_ftrcmob_optuna_tabular(opt_cfg)
    elif opt_cfg["dataset"]["dataset_type"] == "image":
        run_ptrcm_ftrcmob_optuna_image(opt_cfg)
    else:

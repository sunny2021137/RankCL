...
# def rankcl_loss(outputs, labels, n_classes, similarity_metric="cosine", with_correct_penalty=True):
#     # (B, D) -> (N, B//N, D)
...
#     perms = torch.stack([torch.randperm(x_repeated.shape[1]) for _ in range(n_classes)]).to(device=outputs.device)  # shape: (N, B)
...
#     elif similarity_metric == "squaredL2":
...
class RankingAwareContrastiveLoss(nn.Module):
    ...
    def forward(self, outputs, labels):
        ...
        perms = torch.stack([torch.randperm(x_repeated.shape[1]) for _ in range(self.n_classes)]).to(device=outputs.device)  # shape: (N, B)
        # perms = torch.stack([torch.randperm(x_repeated.shape[1], device=outputs.device) for _ in range(self.n_classes)])  # shape: (N, B)
        # (N, B, D)  
        x_repeated = torch.gather(x_repeated, dim=1, index=perms.unsqueeze(-1).expand(-1, -1, x_repeated.shape[-1]))
        # (N, B)

...
def ranking_aware_contrastive_loss(outputs, labels, n_classes, device):
    # (B, D) -> (N, B//N, D)
    ...
    perms = torch.stack([torch.randperm(x_repeated.shape[1]) for _ in range(n_classes)]).to(device)  # shape: (N, B)
    # (N, B, D)  
